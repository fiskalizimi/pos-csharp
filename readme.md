...
* **Type** this is the type of the coupon. It is an enum value and can be:
    ...
    * ```RETURN``` (numeric value of 2) - is when some items are returned. When the coupon type is ```SALE``` the **ReferenceNo** mas be the **CouponId** of the original sale
    * ```CANCEL``` (numeric value of 3) - is when the coupon is cancelled. When the coupon type is ```CANCEL``` the **ReferenceNo** mas be the **CouponId** of the original sale
